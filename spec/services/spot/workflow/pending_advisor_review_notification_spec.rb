...
RSpec.describe Spot::Workflow::PendingAdvisorReviewNotification do
  ...
  let(:work) { create(:student_work, user: depositor, advisor: [approver.user_key], title: ['Test Student Work']) }
  ...
  describe '.send_notification' do
    ...
    context 'with CC users' do
      ...
      it 'sends a message to depositor, "to" user, and all "cc" users' do
        expect { described_class.send_notification(entity: entity, comment: nil, user: approver, recipients: recipients) }
