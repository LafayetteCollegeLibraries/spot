...
RSpec.describe Spot::Derivatives::AvValkyrieDerivativeService, derivatives: true do
  context "when given an audio file" do
    let(:valid_file_metadata) do
      FactoryBot.valkyrie_create(:hyrax_file_metadata, :audio_file, file_set_id: 'abcd1234')
    end
    ...
    describe "#cleanup_derivatives" do
      before do
        allow(Hyrax::DerivativePath).to receive(:derivatives_for_reference)
          .with(file_set)
          .and_return(["/spot/tmp/derivatives/ab/cd/12/34-mp3.mp3"])
        allow(FileUtils).to receive(:rm_f)
        service.cleanup_derivatives
      end

      it 'removes derivative files' do
        expect(FileUtils).to have_received(:rm_f).with("/spot/tmp/derivatives/ab/cd/12/34-mp3.mp3")
      end
    end
  end
  ...
  context "when given an video file" do
    let(:valid_file_metadata) do
      FactoryBot.valkyrie_create(:hyrax_file_metadata, :video_file, file_set_id: 'abcd1234')
    end
    ...
    describe "#create_derivatives" do
      ...
      before do
        allow(service).to receive(:create_thumbnail_from)
      ...
      it 'creates derivative files' do
        expect(service).to have_received(:create_thumbnail_from).with(filename)
    ...
    describe "#cleanup_derivatives" do
      before do
        allow(Hyrax::DerivativePath).to receive(:derivatives_for_reference)
          .with(file_set)
          .and_return(["/spot/tmp/derivatives/ab/cd/12/34-webm.webm", "/spot/tmp/derivatives/ab/cd/12/34-mp4.mp4"])
        allow(FileUtils).to receive(:rm_f)
        service.cleanup_derivatives
      end

      it 'removes derivative files' do
        expect(FileUtils).to have_received(:rm_f).with("/spot/tmp/derivatives/ab/cd/12/34-webm.webm")
        expect(FileUtils).to have_received(:rm_f).with("/spot/tmp/derivatives/ab/cd/12/34-mp4.mp4")
      end
    end
  end
end
