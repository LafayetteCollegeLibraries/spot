...
RSpec.describe Spot::OaiCollectionSolrSet do
  ...
  let(:solr_docs) do
    [
      ...
      { id: 'work_3', has_model_ssim: ['Image'], read_access_group_ssim: ['public'],
        title_tesim: ['An Image'], member_of_collection_ids_ssim: ['collection_2'] }
    ]
  end
  ...
  describe '.sets_for' do
    subject(:sets) { described_class.sets_for(record) }

    let(:record) { SolrDocument.find('work_1') }

    it 'returns the sets that an item belongs to' do
      expect(sets.map(&:spec)).to eq ['collection:collection_1']
    end
  end
