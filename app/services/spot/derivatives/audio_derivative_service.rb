  ...
  module Derivatives
    ...
    class AudioDerivativeService < AudioVisualBaseDerivativeService
      ...
      def rename_premade_derivative(derivative, index)
        ...
        key = format('%s-%d-access.mp3', Hyrax.config.use_valkyrie? ? file_set_resource.id : file_set.id, index)
      ...
      def s3_derivative_keys
        [format('%s-0-access.mp3', Hyrax.config.use_valkyrie? ? file_set_resource.id : file_set.id)]
      end
    end
  end
end
