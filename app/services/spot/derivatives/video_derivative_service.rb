  ...
  module Derivatives
    ...
    class VideoDerivativeService < AudioVisualBaseDerivativeService
      ...
      def s3_derivative_keys
        [format('%s-0-access-1080.mp4', Hyrax.config.use_valkyrie? ? file_set_resource.id : file_set.id),
         format('%s-1-access-480.mp4', Hyrax.config.use_valkyrie? ? file_set_resource.id : file_set.id)]
      end
    end
  end
end
